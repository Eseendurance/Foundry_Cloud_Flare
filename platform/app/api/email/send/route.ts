...
export async function POST(req: NextRequest) {
  ...
  let body: { to?: string; subject?: string; text?: string; linkUrl?: string; linkLabel?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (linkUrl) {
    try {
      ...
      if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        return NextResponse.json({ error: "The link must be a real http(s) URL." }, { status: 400 });
      }
    } catch {
      return NextResponse.json({ error: "The link must be a real http(s) URL." }, { status: 400 });
    }
    link = { url: linkUrl, label: linkLabel || "Open" };
  }

  try {
    const result = await sendEmail({ userId: session.userId, to, subject, text, link });
    return NextResponse.json({
      sent: true,
