...
import { DBClient, TableSchema } from "@/lib/db-client";
...
export default function DatabasePage() {
  ...
  // Sync state with local DBClient after component mounts
  useEffect(() => {
    setTables(DBClient.getTables());
  }, []);
  ...
  async function handleAiSchemaGeneration(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.output) {
        ...
        // Persist new table schema via DBClient
        DBClient.createTable(promptInput, [
          { name: "data_payload", type: "VARCHAR", nullable: true },
          { name: "created_at", type: "TIMESTAMP", nullable: false },
        ]);

        // Refresh state from local persistence store
        setTables(DBClient.getTables());
        setPromptInput("");
      } else {
  ...
  function handleDropTable(tableName: string) {
    const updated = DBClient.dropTable(tableName);
    setTables(updated);
    setStatusLog(`Table "${tableName}" dropped.`);
  }
